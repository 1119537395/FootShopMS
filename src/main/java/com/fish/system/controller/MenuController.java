...
public class MenuController {
	...
	public List<ThreeNode> loadLeftNavigationMenuBar(MenuVo menuVo) {
		...
		List<Menu> list = null;
		// 判断登录用户的类型
		/*if (user.getUserType().equals(MessageEnum.USER_TYPE_SUPER.getCode())) {
			// 系统管理则查询所有的菜单数据并且显示
			list = menuService.queryMenuByAll(menuVo);
		} else {
			// 不是系统管理员则需要根据用户的ID来查询出该用户拥有的菜单数据
			list = menuService.queryMenuByUserId(menuVo,user.getUserId());
		}*/
		// 弃用上面的逻辑判断
		// 改用下面的逻辑判断
		// 所有类型的用户都需要通过ID去查询到拥有的菜单权限
		list = menuService.queryMenuByUserId(menuVo,user.getUserId());
