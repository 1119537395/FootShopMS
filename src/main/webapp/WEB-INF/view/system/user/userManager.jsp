...
<div style="display: none" id="addAndEditModel">
    ...
    <form class="layui-form layui-form-pane" action="" lay-filter="addUserFrame" id="addUserFrame" style="margin: 20px">
        ...
        <%-- 管理员默认头像 --%>
        <%--<input type="hidden" name="userAvatar" id="hiddenUserPicture">--%>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="userName" lay-verify="required" autocomplete="off" class="layui-input" placeholder="请输入用户昵称">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">账户名</label>
            <div class="layui-input-block">
                <input type="test" id="userId" name="userId" style="display: none">
                <input type="text" name="userAccount" lay-verify="required" autocomplete="off" lay-verify="required" class="layui-input" placeholder="请输入用户的账户名">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">手机号</label>
            <div class="layui-input-block">
                <input type="text" name="userPhone" lay-verify="required|number" autocomplete="off" class="layui-input" placeholder="请输入手机号码">
            </div>
        </div>
...
<script>
    ...
    layui.use(['table', 'layer', 'form', 'jquery', 'laydate'], function () {
                ...
                , {
                    field: 'userType', title: '账户类型', align: "center", templet: function (typeData) {
                        ...
                        if (typeData.userType == '1') {
                            return '<font color="blue">前台营业员</font>';
                        }
                    }
                }
        ...
        function resetUserPassword(userData) {
            layer.confirm("确定重置账户【"+userData.userName+"】的密码吗？",{icon:3,title:"提示",skin:'layui-layer-molv',btnAlign:'c'},function () {
                $.post("${Path}/user/resetUserPassword?",{userId:userData.userId},function (returnValue) {
                    if (returnValue.code == 200){
                        layer.msg("成功重置账户【"+userData.userName+"】的密码为[123456]",{icon:6});
                    } else {
                        layer.msg("重置账户【"+userData.userName+"】的密码失败！",{icon:5,anim:6});
                    }
                });
            });
